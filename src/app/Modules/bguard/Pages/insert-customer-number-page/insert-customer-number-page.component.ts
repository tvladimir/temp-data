...
@Component({
  selector: 'bz-insert-customer-number-page',
  templateUrl: './insert-customer-number-page.component.html',
  styleUrls: ['./insert-customer-number-page.component.scss'],
  animations: [
    scaleInOutAnimation
  ]
})
export class InsertCustomerNumberPageComponent implements OnInit, OnDestroy {
  ...
  buildForm() {
    this.verificationForm = this.fb.group({
      ...
      CustomerId: ['', [Validators.required, InputValidator('number')]],
    });
  }
  ...
  reportWT(isOnLoad: boolean, phoneNumber: string, IsSuccessful?: boolean, errorMessage?: string) {
    if (isOnLoad) {
      ...
      if (IsSuccessful) {
        this.webtrendsService.reportWT(new WebTrendsObject({
          action: 'Bguard_mis_lak_continue',
          stage_status: 1,
          errorMsg: 0,
          activity_number: phoneNumber
        }));
